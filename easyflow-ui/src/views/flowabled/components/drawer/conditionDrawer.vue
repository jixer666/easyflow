<template>
    <el-drawer :append-to-body="true" title="条件设置" :visible.sync="conditionDrawer" direction="rtl" class="condition_copyer" size="550px" :before-close="saveCondition">
        <el-select v-model="conditionConfig.priorityLevel" class="priority_level">
            <el-option v-for="item in conditionsConfig.conditionNodes.length" :value="item" :key="item">优先级{{item}}</el-option>
        </el-select>
...
<!--                </ul>-->
                ...
                <div>
                    <el-card v-for="(item,index) in conditionConfig.conditionList" :key="index" class="condition-card">
                        ...
                        <div>
                            <el-form :model="item" label-width="70px">
                                ...
                                <el-form-item label="关系">
                                    <el-select v-model="item.optType">
                ...
                <el-button type="primary" @click="addCondition">添加条件组</el-button>
...
export default {
    ...
    computed: {
        ...mapState('flowabled', ['tableId', 'conditionsConfig1', 'conditionDrawer', 'formItems']),
        attributes() {
